...
class DABstep(QtGui.QMainWindow, user_frontend.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.src_is_Lime = False # make these tristate
        ...
        self.rbtn_Lime.clicked.connect(self.src2Lime)
        ...
        self.t_rbtn_Lime.clicked.connect(self.t_set_sink_Lime)
    ...
    def init_receiver(self):
        if not self.receiver_running:
            ...
            if (not self.src_is_USRP) and (not self.src_is_Lime) and (self.file_path == "None"):
                ...
                if (self.src_is_USRP):
                    self.my_receiver = usrp_dab_rx.usrp_dab_rx(self.spin_dab_mode.value(), 
                                                                ...
                                                                self.bit_rate, self.address, 
                                                                self.size, self.protection, 
                                                                self.audio_bit_rate, self.dabplus, 
                                                                self.src_is_USRP, self.file_path)
                elif (self.src_is_Lime):
                    self.my_receiver = lime_dab_rx.lime_dab_rx(self.spin_dab_mode.value(), 
                                                                self.spinbox_frequency.value(), 
                                                                self.bit_rate, self.address, 
                                                                self.size, self.protection, 
                                                                self.audio_bit_rate, self.dabplus, 
                                                                self.src_is_Lime, self.file_path)
